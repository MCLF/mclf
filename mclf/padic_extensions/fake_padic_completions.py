...
The first advantage of our approach (which addresses the first point above) is
...
class FakepAdicCompletion(SageObject):
    ...
    def weak_splitting_field(self, F):
        ...
        The options should be set such that factors are chosen according to the
        following rules:
        ...
        while True:
            ...
            if isinstance(g, Integer):
                ...
                if first_step and F.degree() == g.degree():
                    # it is often advantageous to replace F by the defining
    ...
    def reduce(self, a, N):
        ...
        `K` has coefficients of the form `c/p^m`, with `0\leq  c < p^N`
        and `m\geq 0`.
    ...
    def minpoly_over_unramified_subextension(self, N):
        ...
        P = x**e - sum(sum(S[e, i+e*j] * zeta[j] for j in range(m)) * x**i
                       for i in range(e))
    ...
    def is_approximate_irreducible_factor(self, g, f, v=None):
        ...
        if g is irreducible over `K` and Krasner's condition is satisfied,
    ...
    def approximate_irreducible_factor(self, f):
        ...
        If an integer `e` is returned, then any tame extension of `K` of ramification
