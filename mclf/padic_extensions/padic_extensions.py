...
This module realizes a class ``pAdicExtension`` which represents a `p`-adic
number field `\hat{K}`. Internally, the `p`-adic number field is represented by
a number field `K` together with a discrete valuation `v_K` on `K` which
extends the `p`-adic valuation `v_p` on `\mathbb{Q}`, such that `\hat{K}` is the
completion of `K` at `v_K`.
...
``Sage``. Therefore, we decided to represent a `p`-adic number field `\hat{K}`
by a pair `(K, v_K)`, where `K` is an *absolute* number field and `v_K` is a discrete
valuation on `K` extending the`p`-adic valuation `v_p` on `mathbb{Q}`, for some
...
Moreover, since there are many pairs `(K, v_K)` which have the same completion,
we can choose `K` in such a way that certain operations we use heavily (like
evaluation of `v_K`) can be done very efficiently. At the moment, we choose
`(K,v_K)` such that

- `v_K` is the unique extension of `v_p` to `K`,
- `K/\mathbb{Q}` is generated by a uniformizer `\pi_K`.

For instance, these assumptions allow us to easily write down a `p`-integral
basis for `K/\QQ`. We can also choose `K` such that the defining polynomial has
small coefficients.

The second advantage is that, since for most of our needs we do not really need
the `p`-adic number field `\hat{K}` explicitly, we can instead work with the
*henselization* `K^h` of `(K,v_K)` which is an *exact* field: an element of `K^h`
is uniquely determined by a finite amount of data, namely its minimal polynomial
over `\mathbb{Q}` and a sufficiently precise approximation by an element of `K`.
Moreover, `K^h` only depends on the `p`-adic number field `\hat{K}` but not
on our particular choice of `K`.

The main drawback of our approach is that morphisms between two `p`-adic number
fields `\hat{K}` and `\hat{L}` are somewhat difficult to realize: the problem is
that for our particular choice of the underlying number fields `K` and `L`, there
may not exist a nonzero morphism `K\to L` even if `\hat{K}` is a subfield of
`\hat{L}`.
...
class pAdicExtension(SageObject):
    r"""
    ...
    - ``vK`` -- a `p`adic valuation on `K`.

    We assume that `v_K` is the unique extension of the `p`-adic extension `v_p`
    on `\mathbb{Q}` to `K`, and that the standard generator `\pi_K` of `K/\mathbb{Q}`
    is a uniformizer for `v_K`. If this is not the case, an error is raised.

    """
    def __init__(self, K, vK):
        assert K.is_absolute(), "K must be an absolute number field"
        assert vK.domain() == K, "K must be the domain of vK"
        p = vK.p()
        ...
        if n > 1:
            ...
            # K = QQ
            # if K is QQ:
            #     # we have to force K to be a number field
            #     K = QQ.extension(x-1, 'z0')
            #     vK = vK.extension(K)
            K = QQ
            piK = K(1)
            e = ZZ(1)
    ...
    def ramification_index(self):
        r"""
        Return the absolute ramification index of this p-adic extension.
        """
    ...
    def extension(self, f, embedding=False):
        ...
        - ``f`` -- a monic univariate polynomial over the number field `K`
          underlying this p-adic extension `\hat{K}`. ``f`` must be irreducible
          over `\hat{K}`.
        - ``embedding`` -- a boolean (default: ``False``)

        OUTPUT:

        A `p`-adic extension `\hat{L}` of `\hat{K}` such that ``f`` has a root
        in an unramified extension of `\hat{L}`, or (if ``embedding`` is ``True``)
        the pair `(\hat{L},\phi)`, where `\phi` is the canonical embedding of
        `\hat{K}` into `\hat{L}`.

        If `K` is the underlying number field, then `f\in K[x]` is irreducible
        over the completion `\hat{K}`; the resulting `p`-adic extension `\hat{L}`
        is a subextension of the `p`-adic number field

        .. MATH::

                \hat{K}[x]/(f).

        However, the number field `L` underlying `\hat{L}` is in general not
        equal to `K[x]/(f)`, and there may not exist any embedding of `K`
        into `L`.
        ...
        for m in range(2,10):
            ...
            if v(pix1) == r:
                break
        # now pix1 is a polynomial over `K` which maps to a uniformizer for the
        ...
        # while True:
        #    P1 = RQ([self.reduce_rational_number(P[i], N) for i in range(n+1)])
        #    if self.is_approximate_irreducible_factor(P1, P):
        #        break
        #    else:
        #        N = N + 1
        # P = P1
        ...
        vL = pAdicValuation(QQ, self.p()).extension(L)
        return pAdicExtension(L, vL)
    ...
    def ramified_extension(self, n, embedding=False):
        ...
        A p-adic extension `L` of `K =` ``self`` such that `L/K` is purely ramified,
        of degree ``n``. If ``embedding`` is ``True`` then the pair `(L,\phi)`
        is returned, where `\phi:K\to L` is the canonical embedding.
        ...
        x = P.parent().gen()
        P = P(x**n)
        L = NumberField(P, 'pi'+str(P.degree()), maximize_at_primes=[])
        vL = self.base_valuation().extension(L)
        return pAdicExtension(L, vL)
    ...
    def weak_splitting_field(self, F):
        ...
        - ``F`` -- a polynomial over the underlying number field `K`,
          or a list of polynomials

        OUTPUT:

        A weak splitting field `\hat{L}/\hat{K}` of ``F``.

        This means that `F` splits over the strict completion `\hat{L}^s` and
        that `\hat{L}^s/\hat{K}^s`.
...
#                   auxiliary functions
    ...
    def is_approximate_irreducible_factor(self, g, f):
        ...
        # this is true if if there is a root beta of f
        # such that vK(alpha-beta)>vK(alpha-alpha_i) for all i
        return result


    def approximate_factorization(self, f):
        r"""
        ...
        factor of `f` over `\hat{K}` and `e>=1`.
        ...
        extension of `\hat{K}` generated by `g` (or `f_i`).
    ...
    def characteristic_polynomial_mod(self, f, N):
        ...
        # to a p-integeral basis of L/QQ, hence BB should be p-integral
        n = K.absolute_degree()
