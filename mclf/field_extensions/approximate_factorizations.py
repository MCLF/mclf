# -*- coding: utf-8 -*-
r"""
Approximate factorization of polynomials over valued fields
===========================================================

Let `(K, v)` be a valued field. We assume that `K` is a *standard field* and
`v` is a nontrivial discrete valuation. It follows that `K` is either a
number field or a function field over a finite or number field. In any case,
`K` is *not* complete with respect to `v`.

Let `L/K` be a finite extension. One of the most important background task within
this project is to compute all extensions of `v` to a discrete valuation on `L`.
Depending on the concrete implementation of the field `K`, this Sage provides
this functionaly via the method :meth:`extensions`. For reasons already partially
explained at the beginning of this section, we would like to have our own
implementation of this task, which gives us more control over the output, and
over some of the intermediate steps of the computation.

Since `K` is a standard field, the finite extension `L/K` is simple, i.e.
generated by one element `\alpha\in L`. Let `f\in K[x]` denote the minimal
polynomial of `\alpha` over `K`. It is well known that the extensions of `v`
to `L` correspond one-to-one to the irreducible factors of `f` over the completion
`K_v` of `K` with respect to `v`.

In this module we implement the concept of an *approximate prime factor*
of a polynomial over a valued field `(K, v)`.
...
def approximate_factorization(K, f, v0=None):
    ...
    - ``K`` - a discretely valued field
    - ``f`` -- a univariate, squarefree polynomial over `K`
    - ``v0`` -- an inductive valuation on `K[x]`, or ``None`` (default: ``None``)
    ...
    from mclf.field_extensions.valued_field_extensions import ValuedField
    assert isinstance(K, ValuedField), "K must be an object of the class ValuedField"
...
def weak_splitting_field(K, f, minimal_ramification=1):
    ...
    This means that `L/K` is a finite extension of discretely valued fields such
    that `f` splits into linear factors over an unramified extension of the fake
    completion of `L`. Moreover, the ramification index of `L/K` is a multiple of
    ...
    f = f.change_ring(K.underlying_field())
    f = f.radical()
    if f.is_constant():
        return K
...
class ApproximatePrimeFactor(SageObject):
    ...
    1. a polynomial over `K` which is
      - monic and integral
      - irreducible over the completion of `K`
    2. an infinite pseudovaluation `v` on `K[x]`
    3. a separating *enhanced* inductive valuation on `K[x]`

    It is clear that a polynomial `f\in K[x]` satisfying the conditions in (1)
    determines a unique pseudovaluation `v_f` on `K[x]` such that `v_f(f)=\infty`.
    Conversely, an infinite pseudovaluation `v` determines a unique irreducible
    polynomial `f\in K_v[x]`, but the coefficients of `f` may not lie in `K`
    ...
    Then by Krasner's Lemma, the completed stem field `L_w=K_v(\alpha)` of `f_1`
    does only depend on the equivalence class of `f_1`.
    ...
    def __init__(self, K, f):
        ...
        from mclf.field_extensions.valued_field_extensions import ValuedField
        assert isinstance(K, ValuedField)
        self._base_field = K
    ...
    def pseudovaluation(self):
        r""" Return the pseudovaluation corresponding to this approximate prime factor.

        """
        raise NotImplementedError()

    def root(self):
        r""" Return the root of the enhanced inductive valuation representing
        this approximate irreducible factor.

        """
        if not hasattr(self, "_root"):
            if hasattr(self, "_pseudo_valuation"):
                raise NotImplementedError()
            else:
                raise NotImplementedError()
        return self._root
    ...
    def exact_polynomial(self):
        r""" Return the irreducible exact polynomial of which this is an
        approximate prime factor.
    ...
    def stem_field(self):
        ...
        OUTPUT: the extension `L/K` of valued fields defined by an
        irreducible polynomial representing this Krasner equivalence class.

        Let `f\in K[x]` be a monic, integral and strongly irreducible polynomial
        lying in this Krasner equivalence class. Then the completion of the stem
        field `L:=K[\alpha]`, where `\alpha` is a root of `f`, is independent,
        up to unique isomorphism, from the choice of the representative `f`.
