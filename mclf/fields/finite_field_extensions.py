...
def finite_field_extension_from_polynomial(K, f):
    ...
    the finite field extension `L:=K[x]/(f)` over `K`.
...
class FiniteExtensionOfFiniteFields(FiniteExtensionOfStandardFields,
                                    StandardFiniteField):
    ...
    - ``phi`` -- an embedding of standard fields `\phi:K\to L`,
                 such that `K` is finite and `L/K` is finite
    - ``M`` -- a simple extension of `K`
    - ``s`` -- a `K`-linear isomorphism `s:L\to M`
    - ``t`` -- the inverse of `s`, `t:M\to L`
...
class FiniteExtensionOfFunctionFields(FiniteExtensionOfStandardFields,
    ...
    This is a fusion of the two classes :class:`StandardFunctionField` and
    :class:`FiniteFieldExtension`. So an object of this class represents
    a function field `L`, considered as a finite extension of another
    function field `K`. Here `K` and `L` are themselves given in standard form.
    ...
    - ``K`` -- a function field, instance of :class:`StandardFunctionField`
...
def is_standard_finite_field_extension(phi):
    ...
    if phi.domain() is phi.codomain() and phi._is_coercion:
        return True
    elif hasattr(phi.codomain(), "base_field"):
...
def standard_model_of_finite_extension(phi):
    ...
    EXAMPLES::
        ...
        sage: L = GF(16)
        sage: phi = K.hom(L)
        sage: M, s, t = standard_model_of_finite_extension(phi)
        sage: M
        Univariate Quotient Polynomial Ring in x over Finite Field in z2
        of size 2^2 with modulus x^2 + x + z2

        sage: K = CyclotomicField(3)
        sage: L = CyclotomicField(9)
        sage: phi = K.hom(L)
        sage: M, s, t = standard_model_of_finite_extension(phi)
        sage: M
    ...
    if not isinstance(phi, EmbeddingOfStandardFields):
        phi = embedding_of_standard_fields(phi)
    K = phi.base_field()       # K and L are in `StandardField` !
    L = phi.extension_field()
    ...
    if K.is_finite():
        ...
        assert L.is_function_field(), "If the domain is a function field,\
            the codomain must be, too"
        return standard_model_of_finite_extension_of_function_fields(phi)


def standard_model_of_finite_extension_of_function_fields(phi):
    r""" Return the standard model of a finite extension of function fields.
    ...
    if not isinstance(phi, EmbeddingOfFunctionField):
        phi = embedding_of_standard_fields(phi)
    K = phi.base_field()       # K and L are in `StandardFunctionField` !
    L = phi.extension_field()

    if phi.is_standard_simple_extension():
        # this means that there is nothing to do: the desired relative model
        # of the extension is simply the original model of L
        M = L.original_model()
        L_to_M = L.to_original_model()
        M_to_L = L.from_original_model()
        return M, L_to_M, M_to_L
    else:
        # we have to construct the relative model
